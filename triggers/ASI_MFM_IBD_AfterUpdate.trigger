trigger ASI_MFM_IBD_AfterUpdate on ASI_MFM_InventoryBalanceDetail__c (after update) {
    ...
    if(CN_IBD.size() > 0){
        ASI_MFM_CN_IBD_TriggerClass.afterUpdateFunction(CN_IBD, trigger.oldMap);
    }
