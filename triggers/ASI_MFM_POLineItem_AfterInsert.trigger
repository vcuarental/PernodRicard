trigger ASI_MFM_POLineItem_AfterInsert on ASI_MFM_PO_Line_Item__c (after insert) {
    ...
    if (Global_RecordTypeCache.getRt(trigger.new[0].recordTypeId).developerName.contains('ASI_MFM_GF')){
        ...
        ASI_MFM_HK_POLineItem_TriggerClass.routineAfterUpsert(trigger.new, trigger.oldmap);
    }
      ...
      && !Global_RecordTypeCache.getRt(trigger.new[0].recordTypeId).developerName.contains('ASI_MFM_SC')) {
        ASI_MFM_POLineItem_TriggerClass.routineAfterUpsert(trigger.new, null);
        ASI_MFM_POLineItem_TriggerClass.routineAfterAll(trigger.new, null); 
    }
    ...
    if(cnPoLineList.size()>0) ASI_MFM_CN_POLine_SetPlan_TriggerClass.routineAfterInsert(trigger.new);
