trigger ASI_HK_CRM_PreApprovalFormItem_BeforeInsert on ASI_HK_CRM_Pre_Approval_Form_Item__c (before insert) {
    if (trigger.new[0].RecordTypeId != null && Global_RecordTypeCache.getRt(trigger.new[0].recordTypeId).developerName.contains('ASI_HK')){    
        ...
        ASI_CRM_TW_PAFItems_TriggerCls.routineBeforeUpsert(trigger.new, null);
    }
}