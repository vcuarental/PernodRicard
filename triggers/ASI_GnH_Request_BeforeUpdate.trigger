/****************************************************************************************************************************
 * Name:ASI_GnH_Request_BeforeUpdate
 * Description: 
 *
 * Version History
...
****************************************************************************************************************************/
trigger ASI_GnH_Request_BeforeUpdate on ASI_GnH_Request__c (before update) {
	...
	if(Global_RecordTypeCache.getRt(trigger.new[0].recordTypeid).developerName.contains('ASI_GnH_TW_Request')) {
        processorList.add(new ASI_TnE_GnH_TW_Validation());
	...
	for(ASI_GnH_Request_TriggerAbstract processor : processorList) {
		...
		processor.executeTriggerAction(ASI_GnH_Request_TriggerAbstract.TriggerAction.BEFORE_UPDATE, trigger.new, trigger.newMap, trigger.oldMap);
