trigger ASI_MFM_POLineItem_BeforeUpdate on ASI_MFM_PO_Line_Item__c (before update) {
    if(trigger.new[0].RecordTypeId != null && Global_RecordTypeCache.getRt(trigger.new[0].recordTypeId).developerName.contains('ASI_MFM_KR')){
        ...
        // [SH] 2018-11-26
        ASI_MFM_TW_POLineItem_TriggerClass.routineBeforeUpsert(trigger.new, trigger.oldmap);
        ...
        ASI_MFM_PH_POLineItem_TriggerClass.routineBeforeUpsert(trigger.new, trigger.oldmap);
    }else if(Global_RecordTypeCache.getRt(trigger.new[0].recordTypeId).developerName.contains('ASI_MFM_VN')){
        // Added by 2019-02-14 Calvin@Laputa
        ASI_MFM_VN_POLineItem_TriggerClass.routineBeforeUpsert(trigger.new, trigger.oldmap);
    ...
    if(cnPoLineList.size()>0) ASI_MFM_CN_POLine_SetPlan_TriggerClass.routineBeforeUpdate(trigger.new, trigger.oldMap); // add oldMap for field check (SOQL 101 issue) @201909
