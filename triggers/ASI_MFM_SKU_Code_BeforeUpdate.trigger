trigger ASI_MFM_SKU_Code_BeforeUpdate on ASI_MFM_SKU_Code__c (before update) {
    map<id, recordType> hkrt_map = new map<id, recordType>([select id from recordType 
            where sobjectType = 'ASI_MFM_SKU_Code__c' and developerName = 'ASI_HK_CRM_SKU']);
        if (hkrt_map.containsKey(trigger.new[0].recordTypeId)) {    
    ...
    //CN RecordType : ASI_FOC_CN_POSM_SKU
        ...
        ASI_MFM_CN_SKU_TriggerClass.beforeUpdateMethod(trigger.new, trigger.oldMap);
    }
