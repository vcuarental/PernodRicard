trigger ASI_CRM_ContractSalesStaff_BeforeInsert on ASI_CRM_ContractSalesStaffIncentive__c  (before insert) {
    if(trigger.new[0].recordTypeId != NULL && Global_RecordTypeCache.getRt(trigger.new[0].recordTypeId).developerName.contains('ASI_CRM_MY')){
        ASI_CRM_MY_ContractSalesStaff_TriggerCls.routineBeforeInsert(trigger.new);
