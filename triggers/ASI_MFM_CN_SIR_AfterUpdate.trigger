trigger ASI_MFM_CN_SIR_AfterUpdate on ASI_MFM_Stock_In_Request__c (after update) {
    ...
    if(CN_SIR_List.size() > 0){
        ASI_MFM_CN_Stock_In_Request_TriggerClass.afterUpdateFunction(CN_SIR_List, trigger.oldMap);
    }
}