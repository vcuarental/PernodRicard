...
({
    ...
    doValidateLInput: function(component,inpAuraId) {
    	// hack to force lightning:input to display correct valid status on blur
    	var doClear = true;
    	var dts = component.find(inpAuraId);
    ...
    doShowInvalid: function(component,inpAuraId) {
    	var dts = component.find(inpAuraId);
    	[].concat(dts).forEach(function(dt) {
    		if(dt){
    			if(dt.get('v.validity') && dt.get('v.validity').valid){
    				dt.set('v.validity',false);
    			}
    			$A.util.addClass(dt, "slds-has-error"); // add red border
		        $A.util.removeClass(dt, "hide-error-message"); // add error message
    		}
